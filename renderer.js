...
const defaults = {
  ...
  encounters: [],
  characters: [],
  map: { ink: '', background: '', stamps: [], paths: [], waterAreas: [], layers: { background: true, grid: true, ink: true, water: true, paths: true, stamps: true }, tool: 'draw', brushColor: '#000000', brushSize: 6, brushStyle: 'marker', pathStyle: 'road', exportScale: 2, exportTransparent: false, gridType: 'square', gridSize: 40 }
};
...
function loadStoredState() {
  try {
    ...
    return parsed && parsed.storagePointer ? {} : parsed;
  } catch (_error) {
...
function mergeState(base, incoming) {
  ...
  next.map.paths = next.map.paths.map(item => ({
    ...item,
    type: item.type === ('ro' + 'ad') || item.type === 'path' ? 'road' : (item.type || 'road'),
    label: item.label === ('Ro' + 'ad') || item.label === 'Path' ? 'Road' : (item.label || pathStyleLabel(item.type || 'road'))
  }));
  next.map.waterAreas = next.map.waterAreas || [];
  next.map.layers = { ...base.map.layers, ...(next.map.layers || {}) };
  next.map.brushStyle = next.map.brushStyle || 'marker';
  next.map.pathStyle = next.map.pathStyle || 'road';
  next.map.exportScale = Number(next.map.exportScale || 2);
  next.map.exportTransparent = Boolean(next.map.exportTransparent);
...
async function saveStateLarge(usePointer = false) {
  ...
  if (!db) {
    showToast('Campaign is too large for browser storage. Save a campaign file.', 'warn');
    return false;
  }
  return new Promise(resolve => {
    const tx = db.transaction(IDB_STORE, 'readwrite');
    tx.objectStore(IDB_STORE).put(appState, IDB_ACTIVE_ID);
    tx.oncomplete = () => {
      if (usePointer) {
...
async function saveStateNow() {
  ...
  try {
    ...
    saveStateLarge(false);
  } catch (_error) {
    const ok = await saveStateLarge(true);
    showToast(ok ? 'Large campaign saved locally.' : 'Campaign is too large for browser storage. Save a campaign file.', ok ? 'info' : 'warn');
  }
}
...
async function hydrateLargeState() {
  ...
  if (!stored) return;
  appState = mergeState(defaults, stored);
...
function readImageFile(input, callback, maxSize = 1400) {
  ...
  if (file.size > 8 * 1024 * 1024) {
    showToast('Choose an image under 8 MB.', 'warn');
    input.value = '';
    return;
  }
...
function openDialog({ title = 'Notice', body = '', fields = [], actions = [{ label: 'OK', value: true, tone: 'primary' }] } = {}) {
  ...
  return new Promise(resolve => {
    ...
    setTimeout(() => (firstField || firstButton)?.focus(), 0);
  });
}
...
function syncMapControlUi() {
  ...
  $$('#path-style, #overlay-path-style').forEach(select => { select.value = appState.map.pathStyle || 'road'; });
...
function pathStyleLabel(value) {
  return ({ road: 'road', trail: 'trail', river: 'river', border: 'border', secret: 'secret route' })[value || 'road'] || 'road';
}
...
async function finishWaterAreaFromButton() {
  if (pendingWaterPoints.length < 3) return;
  const name = await appPrompt('Water area name:', 'Water', 'Water Area');
  if (name === null) return;
  finishWaterArea({ name });
}
...
function placePathPoint(point) {
  ...
  const x2 = point.x;
  const y2 = point.y;
  appState.map.paths.push({ id: uid(), type: appState.map.pathStyle || 'road', label: pathStyleLabel(appState.map.pathStyle), x1, y1, x2, y2, cx: (x1 + x2) / 2, cy: (y1 + y2) / 2 });
  pendingPathPoint = null;
  pathPreviewPoint = null;
  pendingPathConnection = null;
  pendingWaterPoints = [];
...
function renderMapPaths(root = document) {
  ...
  const pathMarkup = (appState.map.paths || []).map(path => {
    ...
    return `<g data-map-path="${path.id}" class="path-path path-${escapeHtml(path.type || 'road')}${selectedClass}"><path d="${pathPathD(path)}"/><text x="${mid.x}" y="${mid.y - 8}">${escapeHtml(path.label || pathStyleLabel(path.type))}</text></g>`;
  }).join('');
...
function applyPathCanvasStyle(ctx, type = 'road') {
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  ctx.strokeStyle = type === 'river' ? '#2778a8' : '#000';
  ctx.lineWidth = type === 'border' ? 7 : type === 'trail' || type === 'secret' ? 6 : type === 'river' ? 11 : 9;
  ctx.setLineDash(type === 'road' ? [22, 14] : type === 'trail' ? [6, 14] : type === 'border' ? [18, 8, 4, 8] : type === 'secret' ? [3, 16] : []);
}
...
function drawExportPaths(ctx) {
  ...
  for (const path of appState.map.paths || []) {
    const { cx, cy } = pathControlPoint(path);
    applyPathCanvasStyle(ctx, path.type || 'road');
    ctx.beginPath();
    ...
    $('#overlay-board').innerHTML = `
      <div class="overlay-map-shell"><aside class="overlay-map-tools" aria-label="Map overlay tools">
        ...
        <div class="overlay-tool-strip overlay-map-adjustments">
          ...
          <select id="overlay-path-style"><option value="road">Road</option><option value="trail">Trail</option><option value="river">River</option><option value="border">Border</option><option value="secret">Secret Route</option></select>
          ...
          <label class="mini-check"><input id="overlay-export-transparent" type="checkbox"> Transparent export</label>
...
function exportData() {
  saveMapInk();
  saveStateNow();
  const payload = { ...appState, fileType: 'dm-drawing-board-campaign', schemaVersion: 8, appVersion: APP_VERSION, savedAt: new Date().toISOString() };
...
function importData(file) {
  ...
  reader.onload = () => {
    try {
      ...
      appAlert('That campaign file could not be opened.', 'Open File');
    }
  };
...
function makeDemoCampaign() {
  ...
  return mergeState(defaults, {
    ...
    campaignPremise: 'A half-sunken border kingdom hides an old crown under blackwater roads, courier shrines, and ruined causeways.',
    scratchpad: 'Demo notes: export a player map, drag pins, curve roads, and try the water lasso.',
    regions: [
      ...
      { id: regionIds[2], name: 'Old Causeway', type: 'Dungeon', tone: '#ruin #travel', sceneTags: ['#road'], secrets: 'Each milestone is a sealed confession.', image: '', x: 65, y: 18 },
    ...
    tasks: {
      hooks: [{ id: taskIds[0], text: 'The ferryman refuses silver and asks for a memory. #arrival' }, { id: taskIds[1], text: 'A courier road vanishes under blue-black water. #road' }],
      scenes: [{ id: taskIds[2], text: 'Lanterns move beneath the surface near Mosslight Fen. #fen #crown' }],
      loose: []
    },
    tables: {
      rumors: [{ id: uid(), text: 'A map drawn in mud changes every dawn.', enabled: true }, { id: uid(), text: 'The queen keeps a road in a bottle.', enabled: true }],
      twists: [{ id: uid(), text: 'The safe route is only safe underwater.', enabled: true }]
    },
    encounters: [{ id: uid(), title: 'Causeway Toll', scenario: 'Three mud-armored knights demand a toll from travelers who still cast shadows.', complication: 'The road sinks 1 foot each round.', reward: 'A milestone key', enabled: true }],
    characters: [{ id: uid(), name: 'Mira Voss', role: 'Salt courier', bio: 'Knows every dry road that no longer exists.', notes: 'Taps twice before lying.', image: '' }],
    map: {
      ...defaults.map,
      brushColor: '#000000',
      brushSize: 8,
      ...
      paths: [
        { id: uid(), type: 'road', label: 'Causeway', x1: 145, y1: 180, x2: 1035, y2: 615, cx: 560, cy: 120 },
...
function bindEvents() {
  ...
  $('#save-btn').addEventListener('click', () => { saveMapInk(); saveStateNow(); showToast('Campaign saved locally.', 'info'); });
