...
function mergeState(base, incoming) {
  ...
  next.map.paths = next.map.paths.map(item => ({
    ...item,
    type: item.type === ('ro' + 'ad') || item.type === 'path' ? 'path' : (item.type || 'path'),
    label: item.label === ('Ro' + 'ad') || item.label === 'Path' ? 'Path' : (item.label || pathStyleLabel(item.type || 'path'))
  }));
  ...
  next.map.pathStyle = next.map.pathStyle || 'path';
